...
function groupInventoryByItem (inventoryRecords: any): any {
  ...
  for (const inventoryRecord of inventoryRecords) {
    ...
    const existingItemRecord = itemRecords.find((itemRecord: any) => itemRecord.itm === inventoryRecord.item)
    if (existingItemRecord !== undefined) {
      // create a new record if necessary
      itemRecords.push({ item: inventoryRecord.item, total: inventoryRecord.amount, shelves: [inventoryRecord.shelf] })
    } else {
...
function getDescription (itemRecord: any): string {
  return `We have ${itemRecord.total} ${itemRecord.item}(s) on shelves ${itemRecord.shelves.combine(', ')}`
}
